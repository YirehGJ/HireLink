...
const JobViewForCandidate = ({ job }: { job: NonNullable<ReturnType<typeof getJob>> }) => {
    return (
        <Card className="max-w-4xl mx-auto">
            <CardHeader>
                <CardTitle>{job.title}</CardTitle>
                <CardDescription>
                    Publicado por <span className="font-medium text-foreground">{job.organizationRef}</span>
                </CardDescription>
            </CardHeader>
            <CardContent>
...
export default function JobDetailsPage({ params }: { params: { id: string } }) {
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 space-y-8">
            <DashboardHeader 
                greeting={role === 'candidate' ? 'Detalles de la Vacante' : `Vacante #${job.id}`}
                title={job.title}
