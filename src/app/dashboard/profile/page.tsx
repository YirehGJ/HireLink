...
import type { Candidate, Organization } from "@/lib/types";
...
export default function ProfilePage() {
    ...
    const { data: candidateProfile, loading: candidateLoading } = useDoc<Candidate>(
        role === 'candidate' && user ? `candidates/${user.id}` : null
    );
    const { data: organizationProfile, loading: orgLoading } = useDoc<Organization>(
        role === 'recruiter' && user?.organizationRef ? `organizations/${user.organizationRef}` : null
    );

    if (!isMounted || !user || candidateLoading || orgLoading) {
        return (
